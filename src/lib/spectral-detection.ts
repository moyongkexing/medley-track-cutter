 ...
 */
export function calculateNoveltyFunction(
  ...
  for (let i = 2; i < features.length - 2; i++) {
    ...
    const centroidChange = weights.spectralCentroid * 
      Math.abs(nextFeature.spectralCentroid - prevFeature.spectralCentroid) / (maxCentroid || 1);
    
    const flatnessChange = weights.spectralFlatness * 
      Math.abs(nextFeature.spectralFlatness - prevFeature.spectralFlatness) / (maxFlatness || 1);
    
    const rolloffChange = weights.spectralRolloff * 
      Math.abs(nextFeature.spectralRolloff - prevFeature.spectralRolloff) / (maxRolloff || 1);
    
    const fluxChange = weights.spectralFlux * 
      Math.abs(nextFeature.spectralFlux - prevFeature.spectralFlux) / (maxFlux || 1);
    
    const rmsChange = weights.rms * 
      Math.abs(nextFeature.rms - prevFeature.rms) / (maxRms || 1);
    
    const zcrChange = weights.zcr * 
      Math.abs(nextFeature.zcr - prevFeature.zcr) / (maxZcr || 1);
 ...
 * @returns 検出された境界時間（秒）
 */
export function detectMusicBoundaries(
  audioBuffer: AudioBuffer, 
  sampleRate: number,
 ...
 */
export function createTracksFromBoundaries(
  boundaries: number[],
  duration: number,
